...
const fetchUsers = async () => {
  try {
    ...
    const response = await fetch(
      `http://localhost:5000/api/messages/users?search=${searchTerm}`
    );
    ...
    // Handle users state error - don't touch messages state here
                ...
                >
                  <div className="message-content">
                    ...
                    {message.sender === 'You' && clickedMessageId === message.id && ( // Show delete button if clicked
                      ...
                      >
                        <i className="pi pi-trash"  style={{ color: 'red' }} /> {/* PrimeIcons trash icon */}
                      </button>
                    )}
                  </div>
                </div>
